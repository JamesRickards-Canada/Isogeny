...
/*SECTION 1: SUPERSINGULAR ELLIPTIC CURVES*/
	...
	addhelp(ssl_graph_scipy,"ssl_graph_scipy(p, l, {jvals}): writes the adjacency matrix of G(p, l) to scipy_adj/p_l.dat, in a format readable by csr_array in scipy.sparse. Can also pass in p=ssl_graph(p, l); alternatively, can supply the list of j-values (useful when comparing with different l's). The file format is three lines of space separated integers, with the first being the data, the second being the rows, and the third being the columns of the corresponding sparse matrix.");
